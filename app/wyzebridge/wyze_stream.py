...
def start_tutk_stream(uri: str, stream: StreamTuple, queue: QueueTuple, state: c_int):
    ...
    # Debug logging for camera authentication data
    camera = stream.camera
    logger.info(f"[{uri}] Camera debug info:")
    logger.info(f"[{uri}] ENR: {'Present' if camera.enr else 'Missing'} ({'***' + (camera.enr[-4:] if camera.enr and len(camera.enr) > 4 else camera.enr or 'None') if camera.enr else 'None'})")
    logger.info(f"[{uri}] MAC: {'Present' if camera.mac else 'Missing'} ({camera.mac})")
    logger.info(f"[{uri}] Parent ENR: {'Present' if camera.parent_enr else 'Missing'} ({'***' + (camera.parent_enr[-4:] if camera.parent_enr and len(camera.parent_enr) > 4 else camera.parent_enr or 'None') if camera.parent_enr else 'None'})")
    logger.info(f"[{uri}] Parent MAC: {'Present' if camera.parent_mac else 'Missing'} ({camera.parent_mac})")
    logger.info(f"[{uri}] DTLS: {camera.dtls}, Parent DTLS: {camera.parent_dtls}")
    ...
    try:
        ...
        logger.warning(f"[{uri}] TUTK Error [{ex.code}] {ex}")
        set_cam_offline(uri, ex, was_offline)
        if ex.code in {-10, -13, -19, -68, -90}:
            exit_code = ex.code
    except ValueError as ex:
        logger.warning(f"[{uri}] ValueError: {ex}")
        if ex.args[0] == "ENR_AUTH_FAILED":
            logger.warning(f"[{uri}] ⏰ Expired ENR? Check camera authentication data.")
            exit_code = -19
        elif "Missing ENR" in str(ex):
            logger.error(f"[{uri}] 🔑 Authentication data missing. Camera may need re-setup in Wyze app.")
            exit_code = -19
    except AttributeError as ex:
        if "'NoneType' object has no attribute 'encode'" in str(ex):
            logger.error(f"[{uri}] 🔑 Camera authentication data is missing (ENR/MAC). Please re-setup camera in Wyze app.")
            exit_code = -19
        else:
            logger.warning(f"[{uri}] AttributeError: {ex}")
    except BrokenPipeError:
        logger.info(f"[{uri}] FFMPEG stopped")
    except Exception as ex:
        logger.warning(f"[{uri}] [{type(ex).__name__}] {ex}")
    else:
