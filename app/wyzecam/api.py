...
_session.verify = "/etc/ssl/certs/ca-certificates.crt"
get = _session.get
post = _session.post
...
def get_camera_list(auth_info: WyzeCredential) -> list[WyzeCamera]:
    ...
    for device in data["device_list"]:
        ...
        # Debug logging for camera data from API
        if nickname:
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"[API DEBUG] Camera '{nickname}' raw data:")
            logger.info(f"[API DEBUG]   ENR: {'Present' if enr else 'Missing'} ({enr})")
            logger.info(f"[API DEBUG]   MAC: {mac}")
            logger.info(f"[API DEBUG]   Parent ENR: {'Present' if parent_enr else 'Missing'} ({parent_enr})")
            logger.info(f"[API DEBUG]   Parent MAC: {parent_mac}")
            logger.info(f"[API DEBUG]   DTLS: {dtls}, Parent DTLS: {parent_dtls}")
            logger.info(f"[API DEBUG]   P2P ID: {p2p_id}")
