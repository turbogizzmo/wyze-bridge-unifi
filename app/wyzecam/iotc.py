...
class WyzeIOTCSession:
    ...
    def _connect(
        ...
        try:
            ...
            if not self.camera.dtls:
                ...
                if not password:
                    raise tutk.TutkError(-1, "Missing ENR for camera authentication")
            ...
            av_chan_id = tutk.av_client_start(
                ...
                (password or "888888").encode("ascii"),
                timeout_secs,
                channel_id,
                resend,
            )
    ...
    def get_auth_key(self) -> str:
        ...
        enr = self.camera.enr
        mac = self.camera.mac
        if not enr or not mac:
            raise tutk.TutkError(-1, f"Missing ENR ({enr}) or MAC ({mac}) for camera authentication")
        auth = enr + mac.upper()
    ...
    def _auth(self):
        ...
        try:
            with self.iotctrl_mux() as mux:
                ...
                if not self.camera.enr:
                    raise tutk.TutkError(-1, "Missing ENR data for camera authentication")
