...
if config.ONVIF_ENABLE:
    from wyzebridge.onvif_server import OnvifServer


class WyzeBridge(Thread):
    __slots__ = "api", "streams", "mtx", "onvif"
    ...
    def __init__(self) -> None:
        ...
        # Initialize ONVIF server if enabled
        self.onvif = None
        if config.ONVIF_ENABLE:
            self.onvif = OnvifServer(self.streams.streams, config.ONVIF_PORT)
            logger.info(f"[ONVIF] Server enabled on port {config.ONVIF_PORT}")
